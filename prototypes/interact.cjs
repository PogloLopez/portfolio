 ...
 *
 *   node prototypes/interact.cjs
 *
 * Serves the whole prototypes/ folder exactly as a static host would, so the
 * root URL is tested too.
 */
...
// Behaves like a static host: a folder URL serves its index.html, and there
// is no directory listing to fall back on.
function serve() {
...
(async () => {
  ...
  try {
    ...
    const landing = `${base}/landing-showcase/`;
    ...
    for (let i = 0; i < SLUGS.length; i++) {
      ...
      await page.mouse.up();
      await page.waitForURL((u) => u.href !== start, { timeout: 5000 }).catch(() => {});
      check(`landing: project ${i + 1}'s 'View full case study' opens its page`, page.url().endsWith(`/project-pages/${SLUGS[i]}.html`), page.url());
    }
    ...
    for (const slug of SLUGS) {
      const url = `${base}/project-pages/${slug}.html`;

      await page.goto(url, { waitUntil: "networkidle" });
      const back = await clickToNavigate(page, page.locator(".topbar a", { hasText: "All work" }), (h) => h.includes("landing-showcase"));
      check(`${slug}: '← All work' goes back to the landing`, /\/landing-showcase\/(index\.html)?#work$/.test(back), back);

      await page.goto(url, { waitUntil: "networkidle" });
      const home = await clickToNavigate(page, page.locator(".topbar a", { hasText: "Pablo A. López" }), (h) => h.includes("landing-showcase"));
      check(`${slug}: the name goes to the landing`, /\/landing-showcase\/(index\.html)?$/.test(home), home);
    ...
    watch(phone, "phone");
    await phone.goto(`${base}/project-pages/rag.html`, { waitUntil: "networkidle" });
    ...
    for (const slug of SLUGS) {
      await phone.goto(`${base}/project-pages/${slug}.html`, { waitUntil: "commit" });
      let worst = 0;
      for (let t = 0; t < 12; t++) {
