/**
 * Self-check for the landing (/): `node tests/verify-landing.cjs`.
 *
 * Drives Chromium through the page on the running site (see lib.cjs) and asserts
...
(async () => {
  ...
  try {
    ...
    const ctas = stops.filter((s) => s.href && s.href.startsWith("/projects/"));
