/**
 * Self-check for the project pages (/projects/<slug>):
 * `node tests/verify-pages.cjs`.
 *
 * Drives Chromium through all five pages on the running site (see lib.cjs) and asserts what a reviewer would
...
const { serve, urls, loadProjects } = require("./lib.cjs");
...
(async () => {
  ...
  try {
    ...
    for (const p of projects) {
      const tag = p.slug;
      await page.goto(urls.project(p.slug), { waitUntil: "networkidle" });
      ...
      const others = await page.evaluate((slug) => {
        ...
        return {
          ...
          self: real.some((a) => a.querySelector(".mcard__cta").getAttribute("href") === `/projects/${slug}`),
        };
      }, p.slug);
    ...
    // Every card link on the landing and on a project page points at a page
    // that exists.
    const links = await page.evaluate(async (base) => {
      const out = [];
      for (const f of ["", "projects/forecast"]) {
    ...
    for (const [name, opts] of [
      ...
      watch(p2, name);
      await p2.goto(urls.project("operations-platform"), { waitUntil: "networkidle" });
